...
@Component({
  selector: 'app-nyu-shukka-zumi-schedule',
  templateUrl: './nyu-shukka-zumi-schedule.component.html',
  styleUrls: ['./nyu-shukka-zumi-schedule.component.scss']
})
export class NyuShukkaZumiScheduleComponent implements OnInit {
  ...
  clickPrevious() {
    ...
    this.router.navigate(['delivery-schedule/nyu-shukka-zumi'], { queryParams: { year: this.viewDate.getFullYear(), month: this.viewDate.getMonth() + 1 } });
    this.populateData();
  }
  clickNext() {
    this.viewDate = new Date(
  ...
  populateData() {
    ...
    this.forecastService.getNyuShukkaForcast(this.viewDate.getFullYear(), this.viewDate.getMonth(), this.selectedPatternId).subscribe(data => {
  ...
  clickOrder(data) {
    const backUrl = {
      base: "delivery-schedule/nyu-shukka-zumi",
